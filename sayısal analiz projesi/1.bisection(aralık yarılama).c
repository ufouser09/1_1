...
double denklem_alma();//asasgidaki kullancagýmýz fonksiyonlarý tanýmladýk
double denklem_hesap(double x);
double aralik_y();


int derece, cevap;//kullanýcagýmýz degiskenleri tanýmladýk
...
double denklem_alma(){
	...
	scanf("%d", &derece);
	while(derece < 0){//eger denklemin derecesi 0 dan kucukse kullanýcýdan denklemin derecesini tekrar girmesini isteriz
	...
	}else{//eger e li bir ifademiz yok ise e nin baskatsayisini 0 yap
		e_katsayi=0;
	}
	...
	if(denklem_katsayi[0][derece]!=0){//denklemin derecesi olan terimi ekrana yazdýrýrz
		printf("(%.2fx^%d)", denklem_katsayi[0][derece], derece);
	}
	for(i=derece-1 ; i>=0 ; i--){//denklemin derecesi olan terim harici olan polinom terimlerini ekrana yazdiririz
...
double denklem_hesap(double x){
	...
	//diðer deðiþkenler
	for(i=0 ; i<=derece ; i++){
		sum = sum + (denklem_katsayi[0][i]*x_sum);//x_sum burda ilk 1 di yani x^0 lý ifademizin katsayisini 0 a attýk sonra x_sum x oldu x^1 li ifademizin katsayisi ile x i carpip bunu sum'a ekledik böylece polinomumuzu bulduk
		x_sum = x_sum * x;
	}
...
double aralik_y(){
	...
	denklem_alma(); // burda denklemimizi aldýk
	...
	if(denklem_hesap(alt)*denklem_hesap(ust) > 0){  //f(alt)*f(ust)<0 ise bu aralýkta kok vardir
		printf("\n(!) Verilen aralik, gereken kosulu saglamamaktadir.");//eger kosul saglanmazsa yöntem uygulanamaz
		printf("\n(!) f(alt) x f(ust) < 0 olmalidir.");
	}else{
		printf("Epsilon degeri: ");// eger ustteki sart saglanýyorsa bu sefer epsilon degeri al
		scanf("%lf", &e);
		
		printf("\nalt\t\tf(alt)\t\tust\t\tf(ust)\t\torta\t\tf(orta)");
		printf("\n---\t\t------\t\t---\t\t------\t\t----\t\t-------");
		
		orta = (alt + ust)/2; //alt ve ust degerin ortasýný bulmak için 2 ye böl ve bunu orta degiskenine ata
		
		while(fabs(denklem_hesap(orta)) >= e){
			
			printf("\n%f\t%f", alt, denklem_hesap(alt));//burda ilk yukarýda buldugumuz ilk alt ust ve orta degerleri ekrana yazdýrýrýz
			printf("\t%f\t%f", ust, denklem_hesap(ust));
			printf("\t%f\t%f", orta, denklem_hesap(orta));
			
			if(denklem_hesap(orta)*denklem_hesap(alt) > 0){//alt ve orta degerlerinin carpýmý pozitifse yani grafikte alt ve orta degerler ayný bölgede ise orta degerini alt degere ata
				alt = orta;
			}else if(denklem_hesap(orta)*denklem_hesap(ust) > 0){ //ust ve orta degerlerin carpimi pozitifse yani grafikte ust ve orta degerler ayný bolgede ise orta degeri ust degere ata
				ust = orta;
			}
		orta = (alt + ust)/2; //yeni buldugumuz degerlerle yeni bir orta deger olustur
		}
